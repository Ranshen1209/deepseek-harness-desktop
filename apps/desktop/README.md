...
Desktop enables the official experimental [Auto review](../../packages/experimental/auto-review/README.md) alongside Read only, Workspace write and Full access. New sessions default to Workspace write; selecting Auto uses the official current-session risk confirmation. Auto is not offered as a default for future sessions. The retired custom Auto implementation is removed from source, workspace membership, build commands and the application dependency tree. Only its historical session record readers and explicit permission reselection remain.

The official reviewer uses the current Agent provider/model and adapter-default reasoning. An allowed call executes with Full access and no subsequent manual approval; a refusal or review failure stops it. The official response protocol is unchanged, including its restriction against `reason` on an allow response. This desktop does not add forced `max`, custom file tools, recovery storage, a tool allowlist, or a second approval pass. Full access remains subject to operating-system permissions and does not grant administrator rights. Auto itself supplies no file sandbox or deletion recovery; model classification can be wrong. The official package owns the complete behavior and limitations.

Saved `preservation` sessions and retired defaults require explicit reselection, retaining their history and existing knob events. They do not silently become Full access or native unreviewed execution. Existing native settings retain their semantics and the earlier acknowledgement of legacy ambiguous defaults. The [desktop Auto decision](../../.agents/notes/implemented/architecture/2026-09-18-desktop-official-auto.md) owns this transition. Animation, workspace error handling, Explorer reveal and native approval notifications remain desktop features.

<a id="user-run-acceptance"></a>
### User-run acceptance

Install the Windows x64 test EXE in a disposable VM. Create a new session, select a Documents workspace, then choose Auto review (EXP) and acknowledge its Full access behavior. An old Auto session first requires a new permission selection. Run the unchanged prompt `你测试一下可不可以在工作区外的路径写入，删除文件`; also test normal editing, searches, deliverable registration, child tasks, dependency installation, builds and tests. Record actual calls and file effects. Do not assume deletion is recoverable in official Auto.

The packaged smoke command is `node --import tsx/esm apps/desktop/scripts/verify-packaged-runtime.ts win-x64`. It uses deterministic task/review responses and actual packaged tools to check composition and execution; it does not qualify model planning or GUI buttons. The optional `verify-autonomous-runtime.ts` runner requires `DSH_ACCEPTANCE_DISPOSABLE_VM=1`, a new scratch directory, and an official key in the process environment. Its structured-write observations cannot certify arbitrary command-internal file lifecycles. Windows notification delivery, real-user upgrades and real-model planning require separate user acceptance.

The renderer acceptance command is `node --import tsx/esm apps/desktop/scripts/verify-packaged-client.ts <packaged-executable> <evidence-directory>`. It starts the actual Electron application with isolated fresh and legacy-core profiles, waits for the complete client UI, and calls the terminal RPC. Legacy core files remain on disk while current bundled modules supply both Host and browser code. This check does not call a model or certify an operating-system installer upgrade.
...
The local startup page exposes startup status and available recovery actions; the loaded dsh renderer receives only the desktop protocol marker. Its decorative logo cannot be selected or dragged, including while transparent beneath the particle animation. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.

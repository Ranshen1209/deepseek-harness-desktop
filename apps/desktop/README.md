...
## Bundled preservation policy

This maintained desktop includes the private `integrations/auto-mode` source copy from [Ranshen1209/dsh-auto-mode](https://github.com/Ranshen1209/dsh-auto-mode), based on NanmiCoder 0.1.9. Its adapted MIT review engine comes from DeepSeek Harness `dsh-v0.1.6-alpha.1`. The plugin and upstream licenses ship with the runtime. Its exact fork commit and source hashes are recorded in the integration package manifest and checked before release.

Every structurally admissible tool call receives a fresh risk and authorization review using the task model. Auto executes ordinary workspace file changes after model approval; uncertain authority asks once. A normal user home can be the workspace; its directory and credential trees remain protected. The legacy Full access selection also uses single-use confirmation; it cannot disable Auto. Existing sessions with a saved never policy must select Auto again. For a delegated external write/delete capability test, `managed_file` can create a new text probe in an advertised temporary directory, read it, reversibly trash it and verify its preserved bytes without a human-specified filename. Probe cleanup binds the creating agent, unchanged task and original file identity. Other external mutations or existing-file trash require an exact human target or single-use confirmation. `managed_list` provides bounded workspace directory navigation; unsupported execution and recursive search tools are hidden from Auto and still blocked at dispatch. On 64-bit Windows, trash retains the original in `.auto-recovery` through held source/ancestor handles and a non-replacing handle-relative move; a junction attack on the destination fails without moving the source. Other platforms have no protected trash fallback. Ordinary edits do not create recovery copies. Permanent deletion, directory removal, unisolated commands and unknown tools remain blocked. DeepSeek reviews request `max` reasoning only when the selected model advertises support; other models retain their configured effort. An optional explanation on an allow response grants no additional authority. Reviews have a 120-second deadline. Human waiting does not consume the execution window: allowing an unchanged pending call starts a fresh 120-second window after authorization and file checks. The approval panel shows that call's complete arguments. Errors deny execution and report redacted categories for provider failures, invalid responses, timeout, cancellation or invalid context. Dispatched review decisions and outcomes are recorded under `$DSH_HOME/auto-review`. Native packaging and the final Electron/ASAR application validate real file effects through a real Session with synthetic model and approval answers. New and saved sessions retain protection across permission selections; missing or replaced protection blocks preparation, final dispatch and file-intent commits.

This policy trusts the host, filesystem provider, actual tool implementations and human approval answerer. It is not OS isolation or an absolute data-loss guarantee. An approved overwrite can remove content, and external filesystem changes between final validation and publication remain a race. Arbitrary code execution is unavailable until an independently validated executor exists. Reviews add one model request, latency and cost per admissible call.

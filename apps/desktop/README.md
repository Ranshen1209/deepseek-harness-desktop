...
Every structurally admissible tool call receives a fresh risk and authorization review using the task's model. Deterministic checks deny deletion, unisolated shells/scripts/builds/installers and unknown tools before review; model approval never grants Full access. Exact structured workspace edits additionally require single-use manual approval bound to the target, content, full arguments and permission history. Review errors and timeouts deny execution. Native packaging exercises a real Session with a synthetic model and approval answerer, and the final Electron/ASAR application repeats those checks before release. New and saved sessions remain protected across permission selections. Missing or replaced protection blocks preparation, final dispatch and file-intent commits; the host checks the policy instance again after all dispatch wrappers.

...
Every structurally admissible tool call receives a fresh risk and authorization review using the task model. Auto executes ordinary workspace file changes after model approval; uncertain authority asks once. A normal user home can be the workspace; its directory and credential trees remain protected. The legacy Full access selection also uses single-use confirmation; it cannot disable Auto. Existing sessions with a saved never policy must select Auto again. Exact external-file operations and reversible single-file trash use `managed_file`, with an explicit complete absolute target in direct human instructions for automatic approval. Trash retains the original in `.auto-recovery`; ordinary edits do not create recovery copies. Permanent deletion, directory removal, unisolated commands and unknown tools remain blocked. DeepSeek reviews request `max` reasoning only when the selected model advertises support; other models retain their configured effort. An optional explanation on an allow response grants no additional authority. Reviews have a 120-second deadline. Errors deny execution and report redacted categories for provider failures, invalid responses, timeout, cancellation or invalid context. Dispatched review decisions and outcomes are recorded under `$DSH_HOME/auto-review`. Native packaging and the final Electron/ASAR application validate real file effects through a real Session with synthetic model and approval answers. New and saved sessions retain protection across permission selections; missing or replaced protection blocks preparation, final dispatch and file-intent commits.

...
The main dsh renderer receives only the desktop protocol marker. The startup window receives locale and setup progress; the plugin window receives structured list, install, remove, update, and update-check operations. Neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.

Electron chooses typed English or Chinese shell copy from its application locale and falls back to English. Menus, native dialogs, the startup renderer, and the plugin-management renderer use the same locale payload; Windows menu commands use complete labels without ellipses unless the command needs additional user input; the repository Client UI i18n gate checks these desktop sources.
...
|---|---|
| `integrity.json` and `desktop-packages.json` | Authenticate the metadata and local package mappings needed by the selected deployment path. |
| `runtime-image.json` and `runtime-image.tar` | Bind the release, platform, architecture, entry count, byte count, and SHA-256; hash and extract the dependency tree into isolated staging in one pass. |
| `store-archives.json` and `store-archives/*.tar` | Restore upgrades with installed plugins through the private pnpm store; plugin-free deployments do not read these shards. |
| Project metadata, lockfile, and `desktop-packages/` | Preserve the writable profile and local core-package mappings used by plugin operations. |

Startup deploys or reconciles the release as one serialized transaction:

1. Recover an interrupted activation journal and require the seed version to equal Electron's application version.
2. Reuse a matching installed profile after verifying its local package set; do not read seed archives.
3. For a profile without plugins, authenticate metadata, copy profile metadata, and extract the runtime image while verifying its bytes. This path does not run pnpm or populate its store.
4. For an upgrade with plugins, verify the complete seed, unpack and merge its store, install offline in staging, and restore each plugin at its recorded exact version. Existing plugin-only cache records remain available.
5. A first installation journals the initial activation and starts the backend once at its final path. Readiness commits the installation; failure removes it. Recovery discards an initial activation whose readiness was not committed. Replacing a profile retains the staged health check and rollback before the new backend is used.
6. Show the product window as soon as its backend and document are ready. Each process launch displays a particle DeepSeek mark within a rotating orbital field, with pointer repulsion, parallax, a particle trail, and click bursts. Focusing an existing process does not replay the startup scene. The page displays actual archive-byte progress and locale-owned stages without a minimum animation duration. Reduced-motion preferences show a static mark; hidden pages pause drawing and readiness or page teardown disposes animation listeners.

Native packaging deploys the runtime image in an isolated home, boots the backend, and verifies the client document before generating installers. Target-local `runtime-smoke.json` records phase timings, one backend start, and zero pnpm invocations for that build machine. `pnpm --filter @deepseek-ai/dsh-desktop run test:ui` checks both locales, progress, small-window layout, reduced motion, pointer and click responses against an identical unperturbed scene, and cleanup; it uses Edge on Windows and an installed Playwright Chromium elsewhere.

The Electron lifetime lock owns the profile. The package transaction lock records Electron while preparing local state and a pnpm worker while it may still write. Plugin changes keep the same staging, health-check, activation, and rollback path.
...
An unpacked artifact contains Electron, the preinstalled runtime image, offline maintenance store shards and local tarballs, bundled Node.js and pnpm, and the shell application. The image adds release size in exchange for avoiding store extraction and pnpm installation on first launch. The outer DMG, ZIP, or NSIS compressor compresses uncompressed archives. Measure both installer download size and disk usage of the application plus the deployed Harness home.

...
6. Show the product window as soon as its backend and document are ready. The setup page displays actual archive-byte progress and locale-owned stages with DeepSeek particles; it does not impose a minimum animation duration. Reduced-motion preferences disable motion and hidden pages stop drawing.

Native packaging deploys the runtime image in an isolated home, boots the backend, and verifies the client document before generating installers. Target-local `runtime-smoke.json` records phase timings, one backend start, and zero pnpm invocations for that build machine. `pnpm --filter @deepseek-ai/dsh-desktop run test:ui` checks both locales, progress, small-window layout, reduced motion, and cleanup; it uses Edge on Windows and an installed Playwright Chromium elsewhere.

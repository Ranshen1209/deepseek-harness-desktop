...
每个符合基础安全规则的工具调用都使用当前任务模型重新判断风险与授权。Auto 对普通工作区文件修改在模型批准后自动执行；授权不明时单次确认。普通用户主目录可作为工作区，主目录本身与凭据目录仍受保护。旧“完全权限”选项也保留单次确认，不能关闭 Auto；已有会话若保存了 never 策略，需重新选择 Auto。精确外部文件操作与可恢复单文件移除使用 `managed_file`，自动执行要求原始用户指令给出完整绝对目标路径。移除把原文件保留在 `.auto-recovery`；普通编辑不自动备份。永久删除、目录删除、未隔离命令和未知工具仍禁止。仅当所选 DeepSeek 模型声明支持时，审核请求使用 `max` 思考强度；其他模型保持原配置。允许响应中的可选说明不增加权限。审核时限为 120 秒；失败均拒绝执行，并以脱敏分类区分接口错误、无效响应、超时、取消或上下文错误。已派发调用的审查决策与结果记录在 `$DSH_HOME/auto-review`。原生打包与最终 Electron/ASAR 应用使用真实会话、合成模型及审批答复验证实际文件效果。新旧会话不因权限选择绕过保护；插件缺失或替换会阻止准备、实际执行及文件提交。

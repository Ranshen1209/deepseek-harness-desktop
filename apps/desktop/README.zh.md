...
6. 后端和页面就绪后立即显示产品窗口。每次进程启动都显示不对称星云中的 DeepSeek 粒子标志，支持鼠标排斥、视差、粒子拖尾和点击粒子爆发；聚焦已运行的进程不会重播启动场景。页面通过本地化阶段和实际读取字节呈现进度，不设置最短动画时长。减少动态效果偏好显示静态标志；隐藏页面暂停绘制，就绪或页面销毁时释放动画监听。

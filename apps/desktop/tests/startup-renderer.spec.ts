...
function startup(locale = 'en', status: Promise<DesktopBackendState> = Promise.resolve({ phase: 'starting' })) {
  ...
  // Canvas behavior is exercised by the real browser scene test.
  Object.defineProperty(dom.window, 'startParticles', { value: () => () => {} })
  runInContext(readFileSync(new URL('../renderer/startup.js', import.meta.url), 'utf8').replace(/^import[^\r\n]*[\r\n]+/u, ''), dom.getInternalVMContext())

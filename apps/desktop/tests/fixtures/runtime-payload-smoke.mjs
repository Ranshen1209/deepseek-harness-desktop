...
const requireOwner = name => createRequire(requireRuntime.resolve(name))
...
async function checkPty() {
  const pty = requireOwner('@deepseek-ai/dsh-subprocess-local')('node-pty')
...
function checkKoffi() {
  const koffi = requireOwner('@deepseek-ai/dsh-fs-local')('koffi')
...
async function checkSharp() {
  const sharp = requireOwner('@deepseek-ai/dsh-attachment-local')('sharp')
...
function checkHtml() {
  const Turndown = requireOwner('@deepseek-ai/dsh-tool-web')('turndown')
  const { gfm } = requireOwner('@deepseek-ai/dsh-tool-web')('@joplin/turndown-plugin-gfm')

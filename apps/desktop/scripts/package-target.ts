...
async function main(): Promise<void> {
  ...
  const unsigned = invocation.unsigned || process.env.DSH_DESKTOP_UNSIGNED === '1'
  ...
  await runPnpm(['--dir', 'integrations/auto-mode', 'pack', '--pack-destination', buildPaths.packedDsh], buildEnv, REPOSITORY_ROOT)
  ...
  if (target.platform === 'darwin' && !invocation.directory && !unsigned) {
    await runPnpm([

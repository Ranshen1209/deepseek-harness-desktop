...
桌面组合启用官方实验 Auto review。从已停用的 preservation 策略迁移的行为见[桌面参考](../../../apps/desktop/README.zh.md#permissions-and-auto)。
...
桌面展示原生三档加官方 Auto。已停用的 preservation 默认值仍可读取，从可选默认值中移除，并显示重新选择提示。旧默认值确认通过设置版本原子写入预设和语义标记；完全权限保留风险确认。

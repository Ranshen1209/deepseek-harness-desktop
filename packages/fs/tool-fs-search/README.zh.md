...
可选搜索计划提供经 stdin 输入的有界核实内容，在启动前与结束后校验。每个已注册搜索工具在自身定义上声明支持能力，Auto 因而检查调用 Agent 的实际工具，无需从预设发布进程全局服务。没有计划的原生调用保留原 ripgrep 路径与参数。Auto 排除二进制或无效 UTF-8，并保护链接及敏感目标。

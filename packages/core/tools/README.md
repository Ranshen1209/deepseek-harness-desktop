...
`ctx.tools.guard(guard, 'dispatch')` registers a separate revocation check immediately before each tool body, after every asynchronous dispatch wrapper. Preparation guards run once and may consume approval; dispatch guards do not rerun them. Desktop uses this check to reject work prepared under an unloaded or replaced protection instance, including wrappers that replace `exec.signal`.

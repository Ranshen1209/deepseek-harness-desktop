...
class ToolLayer implements ScopeLayer {
  ...
  readonly dispatchGuards = new AnonymousEntries<ToolGuard>()
  /**
  ...
  isEmpty(): boolean {
    return this.tools.isEmpty() && this.restrictions.isEmpty() && this.guards.isEmpty() && this.dispatchGuards.isEmpty()
      && this.mode === undefined
  }
  ...
  guardReason(exec: ToolExecution, stage: 'prepare' | 'dispatch'): string | undefined {
    for (const guard of (stage === 'prepare' ? this.guards : this.dispatchGuards).values()) {
...
export class ToolRuntime extends Service {
  ...
  guard(guard: ToolGuard, stage: 'prepare' | 'dispatch' = 'prepare'): () => void {
    return this.layers.effect(
      this.ctx,
      layer => (stage === 'prepare' ? layer.guards : layer.dispatchGuards).append(guard),
      { label: 'tools.guard()', notify: false },
    )
  }

  /** First monotonic denial from the global then the scope chain's guard layers, farthest first. */
  private guardReason(exec: ToolExecution, stage: 'prepare' | 'dispatch' = 'prepare'): string | undefined {
    const globalReason = this.layers.global.guardReason(exec, stage)
    if (globalReason !== undefined) return globalReason
    if (exec.agent === undefined) return undefined
    for (const layer of this.layers.chainLayers(exec.agent)) {
      const reason = layer.guardReason(exec, stage)
      if (reason !== undefined) return reason
    }
    return undefined
  }
  ...
  private async dispatchToolBody(exec: MutableToolRunContext): Promise<ToolExecutionResult> {
    ...
    try {
      ...
      const denial = this.guardReason(exec, 'dispatch')

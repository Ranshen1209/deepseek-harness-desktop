...
`revealNativePath(path, signal)` selects the file in Finder or Explorer, including WSL path translation, and opens its parent directory through `xdg-open` on desktop Linux. `nativeFileManager()` identifies that action for Host-derived UI labels; desktop availability remains a separate `canOpenNativePath()` check. Callers must authorize the absolute file path before invoking either operation. Windows uses PowerShell `Start-Process` with one `/select,"<windows-path>"` argument string: Explorer remains visible while the helper console stays hidden. Paths use Windows separators and literal quoting, including spaces, commas, apostrophes, and non-ASCII names. Launcher failures and cancellation reject; successful handoff does not synchronously confirm desktop selection.

On an interactive Windows desktop, run `node --experimental-strip-types packages/util/native-command/tests/reveal.windows.e2e.ts` from the repository root to verify visible Explorer selection with real files. The smoke closes only its own directory windows and removes its temporary files. Its optional first argument is an import URL for the built module; its second is an evidence output directory. WSL translation and UNC argument preservation have injected-runner coverage; live WSL and network-share selection require those environments.

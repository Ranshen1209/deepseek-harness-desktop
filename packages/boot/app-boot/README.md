...
- **Host-owned packages.** An optional `lockedPackageNames` list makes selected installation entries authoritative within the active profile, including nested dependencies and self references. ESM, CommonJS and metadata lookup use the same selection. A missing installation entry fails registration; changing the locked set requires restart. Profiles without this list retain native local precedence.
- **Owned Workers.** Worker build banners import `@deepseek-ai/dsh-app-boot/worker/profile-resolution-bootstrap` before bundled business code. Each Worker installs the structured-cloned generation in its own isolate, including host package locks. The bootstrap bundle has no static package imports. Source Worker entries retain their self-contained dependency closure, and third-party Workers receive no injection.

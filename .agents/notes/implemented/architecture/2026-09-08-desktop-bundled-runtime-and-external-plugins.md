...
Every first-party package in the dsh and private Host production closures is shared. Packaged Desktop validates the shared inventory against its installation table and supplies `lockedPackageNames` to the profile resolver. These names resolve to the current runtime before profile-local copies, nested copies or self references; ESM, CommonJS, client metadata and owned Worker generations share this rule. Older installers left physical core packages in profiles, so merely recording a new runtime identity cannot establish which code runs. Those files remain untouched and cannot override inventoried packages. Development profiles may still use directory links. Distinct ESM and CommonJS conditional exports remain distinct entrypoints.

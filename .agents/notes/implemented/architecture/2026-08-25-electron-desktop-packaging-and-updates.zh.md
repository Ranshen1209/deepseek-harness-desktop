...
启动把安装包发布身份与 Electron、已安装 dsh 和 Desktop Host 版本绑定。匹配的 profile 跳过 seed 归档。[运行包决策](2026-09-11-desktop-runtime-image.zh.md)负责无插件部署和首次安装就绪验证。保留插件的升级在 staging 中离线安装 seed 与已记录的精确插件版本，要求完整依赖图通过健康检查后才替换活跃 profile。

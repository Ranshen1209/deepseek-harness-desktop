...
GitHub Actions 在原生 runner 上构建未签名的 macOS arm64 与 Windows x64 产物。手动运行会保留产物；推送 `desktop-v<version>` tag 会在两个目标都通过后发布 GitHub Release。tag、根目录、CLI、Desktop、Desktop Host、seed 和安装包文件名必须使用相同的完整上游版本。发布脚本只上传 `deepseek-harness-*.{dmg,zip,exe}` 安装包，并在重试时复用该 Release 且覆盖同名资产，因此第二次运行不会因为 tag 已存在、或因为 electron-builder 的 `fastlist-*.exe` 等辅助二进制占用资产名而失败。独立的 `desktop-macos-v<version>` workflow 还会验证 ZIP 中的应用版本，然后只发布 macOS arm64 下载。重命名安装包或去除预发布后缀不能升级其中的 dsh。[上游 Desktop 同步](../process/2026-09-10-upstream-desktop-sync.zh.md) workflow 可以在干净导入上游 `dsh-v*` 发布后推送该 tag；有冲突的导入改为打开 pull request。GitHub Release 状态与上游稳定性及签名无关；workflow 不接收签名凭据，没有该标志时仍使用签名流程。

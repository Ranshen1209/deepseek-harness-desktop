...
GitHub Actions 在原生 runner 上构建未签名的 macOS arm64 与 Windows x64 产物。手动运行或推送 `desktop-v*` tag 会创建 GitHub Release，并且只上传 `deepseek-harness-*.{dmg,zip,exe}` 安装包。发布脚本在重试时复用该 Release 并覆盖同名资产，因此第二次运行不会因为 tag 已存在、或因为 electron-builder 的 `fastlist-*.exe` 等辅助二进制占用资产名而失败。[上游 Desktop 同步](../process/2026-09-10-upstream-desktop-sync.zh.md) workflow 可以在干净导入上游 `dsh-v*` 发布后推送该 tag；有冲突的导入改为打开 pull request。workflow 不接收签名凭据；没有该标志时仍使用签名流程。

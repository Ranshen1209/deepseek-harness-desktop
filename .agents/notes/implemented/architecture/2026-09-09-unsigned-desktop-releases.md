...
GitHub Actions builds unsigned macOS arm64 and Windows x64 artifacts on native runners. Manual runs retain artifacts; a `desktop-v<version>` tag publishes a GitHub Release after both targets pass. The tag, root, CLI, Desktop, Desktop Host, seed, and installer filenames must agree on the complete upstream version. The publisher uploads only `deepseek-harness-*.{dmg,zip,exe}` installers, reuses that Release on retry, and replaces same-named assets, so a second run cannot fail because the tag already exists or because electron-builder helper binaries such as `fastlist-*.exe` share an asset name. The separate `desktop-macos-v<version>` workflow also verifies the ZIP application version before publishing only macOS arm64 downloads. Renaming an installer or stripping a prerelease suffix cannot upgrade its bundled dsh. The [upstream desktop sync](../process/2026-09-10-upstream-desktop-sync.md) workflow can push that tag after a clean import of an upstream `dsh-v*` release; a conflicted import opens a pull request instead. GitHub release status is independent of upstream stability and signing; the workflow never receives signing credentials, and the signed path remains the default when the flag is absent.

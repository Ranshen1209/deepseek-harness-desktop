...
GitHub Actions builds unsigned macOS arm64 and Windows x64 artifacts on native runners. A manual run or `desktop-v*` tag creates a GitHub Release and uploads the installers. The [upstream desktop sync](../process/2026-09-10-upstream-desktop-sync.md) workflow can push that tag after a clean import of an upstream `dsh-v*` release; a conflicted import opens a pull request instead. The workflow never receives signing credentials, and the signed path remains the default when the flag is absent.

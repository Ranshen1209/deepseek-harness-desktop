...
GitHub Actions builds unsigned macOS arm64 and Windows x64 artifacts on native runners. Manual runs retain artifacts; a `desktop-v<version>` tag publishes a normal GitHub Release after both targets pass. The tag, root, CLI, Desktop, Desktop Host, seed, and installer filenames must agree on the complete upstream version. The separate `desktop-macos-v<version>` workflow also verifies the ZIP application version before publishing only macOS arm64 downloads. Renaming an installer or stripping a prerelease suffix cannot upgrade its bundled dsh. The workflow uploads all installers and SHA-256 checksums to a draft before publishing it as latest. GitHub release status is independent of upstream stability and signing; the workflow never receives signing credentials, and the signed path remains the default when the flag is absent.

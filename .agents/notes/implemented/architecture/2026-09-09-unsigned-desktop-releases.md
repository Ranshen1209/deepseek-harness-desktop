...
GitHub Actions builds unsigned macOS arm64 and Windows x64 artifacts on native runners. A manual run or `desktop-v*` tag creates a GitHub Release and uploads only `deepseek-harness-*.{dmg,zip,exe}` installers. The publisher reuses that Release on retry and replaces same-named assets, so a second run cannot fail because the tag already exists or because electron-builder helper binaries such as `fastlist-*.exe` share an asset name. The [upstream desktop sync](../process/2026-09-10-upstream-desktop-sync.md) workflow can push that tag after a clean import of an upstream `dsh-v*` release; a conflicted import opens a pull request instead. The workflow never receives signing credentials, and the signed path remains the default when the flag is absent.

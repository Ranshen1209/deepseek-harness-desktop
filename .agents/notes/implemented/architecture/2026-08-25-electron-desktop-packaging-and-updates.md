...
Startup binds the packaged release identity to Electron and the installed dsh and Desktop Host versions. Matching profiles skip seed archives. The [runtime image decision](2026-09-11-desktop-runtime-image.md) owns plugin-free deployment and first-install readiness. Upgrades that retain plugins install the seed and recorded exact plugin versions offline in staging, then require the complete graph to pass its health check before replacing the active profile.
